...
def generate_topk_views(k, data):
    ...
    for a,m,f in views:
        #print (a,f,m)
        q1="select "+a+", "+f+"("+m+") from df_param_passing where cp = 'typical_angina' and "+a+" is not null group by "+a+" order by "+a+";"
        q2="select "+a+", "+f+"("+m+") from df_param_passing where cp != 'typical_angina' and "+a+" is not null group by "+a+" order by "+a+";"
        #print(q1)
...
def missing_data(db, percentage, seed):
    ...
    columns = ['age', 'sex', 'restbp', 'chol', 'fbs', 'restecg', 'thalach',
...
discrete=['sex', 'num', 'fbs', 'restecg', 'exang', 'slope', 'thal']
