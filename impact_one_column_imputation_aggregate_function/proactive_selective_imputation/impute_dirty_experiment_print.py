    ...
    for i in mlist:
        for j in range(1):
        	for m in m_list:
	            ...
	            missing_topk = generate_topk_views(k+m, data)

	            unique_dirty = get_unique(k+m, missing_topk)
	            #print(k+m)
	            #print(unique_dirty)
	            unique_col = [ item for elem in unique_dirty for item in elem]
	            #print("Unique col first iteration", unique_col)

	            for col in unique_col:
	                data.drop([col], axis=1, inplace=True)
	                data[col] = df[col]

	            imputed_missing_topk = generate_topk_views(k, data)
	            # print("Imputed cols", unique_col)
	            # print("Dirty topk")
	            # print(imputed_missing_topk)
	            sum_imputed_missing = get_utility_score_missing_topk(k, ideal_topk, imputed_missing_topk)
	            #fields = [rboresult(imputed_missing_topk, ideal_topk), jaccard_similarity(imputed_missing_topk, ideal_topk), cum_score(sum_imputed_missing,sum_ideal)]
	            print("RBO", fields[0])
	            print("\n")
	            print("Jaccard", fields[1])
	            print("\n")
	            print("CD", fields[2])
	            # with open('results/dirty_impute_additional.csv', 'a', newline='') as f:
		           #  #print(missing_topk, len(missing_topk))
		           #  #print(topk, len(topk))
		           #  fields = [i*100, k, m, rboresult(imputed_missing_topk, ideal_topk), jaccard_similarity(imputed_missing_topk, ideal_topk), cum_score(sum_imputed_missing,sum_ideal)]
		           #  print(fields)
		           #  writer = csv.writer(f)
            ...
            # # if fields[1] < 1:
            # #     unique_dirty1 = get_unique(k, imputed_missing_topk)
            # #     unique_col1 = [ item for elem in unique_dirty1 for item in elem]
            # #     print("While iteration unique col", unique_col1)

            # #     for col in unique_col1:
            # #         data.drop([col], axis=1, inplace=True)
            # #         data[col] = df[col]
            # #     imputed_missing_topk1 = generate_topk_views(k, data)
            # #     jaccard = jaccard_similarity(imputed_missing_topk1, ideal_topk)
            ...
            #     jaccard = jaccard_similarity(imputed_missing_topk1, ideal_topk)
            #     print("Jaccard result", jaccard)
            #     imputed_missing_topk = imputed_missing_topk1



            # with open('results/dirty_impute.csv', 'a', newline='') as f:
            #     #print(missing_topk, len(missing_topk))
            #     #print(topk, len(topk))
            #     fields = [i*100, k, rboresult(imputed_missing_topk, ideal_topk), jaccard_similarity(imputed_missing_topk, ideal_topk), cum_score(sum_imputed_missing,sum_ideal)]
            #     print(fields)
            #     writer = csv.writer(f)
            #     writer.writerow(fields)
