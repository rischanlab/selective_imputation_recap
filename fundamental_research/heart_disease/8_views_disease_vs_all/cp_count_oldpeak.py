...
for i in mlist:
	for j in range(100):
		...
		q1="select cp, count(*) from df where num = 'disease' and cp is not null and oldpeak is not null group by cp order by cp;"
		q2="select cp, count(*) from di1 where num = 'disease' and cp is not null and oldpeak is not null group by cp order by cp;"


		res_1=get_query_result(q1)
		res_1.columns = ['heart_disease','count_oldpeak']
		res_1['count_oldpeak'] = res_1['count_oldpeak']/sum(res_1['count_oldpeak'])
		res_1 = res_1.set_index('heart_disease')
		res_2=get_query_result(q2)
		res_2.columns = ['heart_disease','count_oldpeak']
		res_2['count_oldpeak'] = res_2['count_oldpeak']/sum(res_2['count_oldpeak'])
		res_2 = res_2.set_index('heart_disease')

		dist = 1 - get_utility(res_1['count_oldpeak'],res_2['count_oldpeak'])
		with open('results/count_oldpeak_distance.csv', 'a', newline='') as f:
			fields = [i*100, dist]
			print(fields)
			writer = csv.writer(f)
