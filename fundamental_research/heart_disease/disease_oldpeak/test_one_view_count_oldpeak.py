...
for i in mlist:
	for j in range(100):
		...
		q1="select num, count(*) from df where cp = 'typical_angina' and num is not null group by num order by num;"
		q2="select num, count(*) from di1 where cp = 'typical_angina' and num is not null group by num order by num;"
