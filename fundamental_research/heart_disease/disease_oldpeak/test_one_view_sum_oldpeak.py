...
for i in mlist:
    for j in range(100):
        ...
        q1="select num, sum(oldpeak) from df where cp = 'typical_angina' and num is not null and oldpeak is not null group by num order by num;"
        q2="select num, sum(oldpeak) from di1 where cp = 'typical_angina' and num is not null and oldpeak is not null group by num order by num;"


        res_1=get_query_result(q1)
        res_1.columns = ['heart_disease','sum_oldpeak']
        res_1['sum_oldpeak'] = res_1['sum_oldpeak']/sum(res_1['sum_oldpeak'])
        res_1 = res_1.set_index('heart_disease')
        res_2=get_query_result(q2)
        res_2.columns = ['heart_disease','sum_oldpeak']
        res_2['sum_oldpeak'] = res_2['sum_oldpeak']/sum(res_2['sum_oldpeak'])
        ...
        dist = 1 - get_utility(res_1['sum_oldpeak'],res_2['sum_oldpeak'])
        with open('results/sum_oldpeak_distance.csv', 'a', newline='') as f:
            fields = [i*100, dist]
            print(fields)
            writer = csv.writer(f)
