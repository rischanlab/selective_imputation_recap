...
def dropout(a, percent, seed):
    # create a copy
    mat = a.copy()
    # restecgber of values to replace
...
for i in mlist:
	for j in range(100):
		...
		q1="select restecg, avg(chol) from df where cp = 'typical_angina' and restecg is not null and chol is not null group by restecg order by restecg;"
		q2="select restecg, avg(chol) from di1 where cp = 'typical_angina' and restecg is not null and chol is not null group by restecg order by restecg;"


		res_1=get_query_result(q1)
		res_1.columns = ['heart_disease','avg_chol']
		res_1['avg_chol'] = res_1['avg_chol']/sum(res_1['avg_chol'])
		res_1 = res_1.set_index('heart_disease')
		res_2=get_query_result(q2)
		res_2.columns = ['heart_disease','avg_chol']
		res_2['avg_chol'] = res_2['avg_chol']/sum(res_2['avg_chol'])
		res_2 = res_2.set_index('heart_disease')

		dist = 1 - get_utility(res_1['avg_chol'],res_2['avg_chol'])
		with open('results/avg_chol_distance.csv', 'a', newline='') as f:
			fields = [i*100, dist]
			print(fields)
			writer = csv.writer(f)
