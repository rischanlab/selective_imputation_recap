...
for i in mlist:
	for j in range(100):
		...
		q1="select num, avg(oldpeak) from df where cp = 'typical_angina' and num is not null and oldpeak is not null group by num order by num;"
		q2="select num, avg(oldpeak) from di1 where cp = 'typical_angina' and num is not null and oldpeak is not null group by num order by num;"


		res_1=get_query_result(q1)
		res_1.columns = ['heart_disease','avg_oldpeak']
		res_1['avg_oldpeak'] = res_1['avg_oldpeak']/sum(res_1['avg_oldpeak'])
		res_1 = res_1.set_index('heart_disease')
		res_2=get_query_result(q2)
		res_2.columns = ['heart_disease','avg_oldpeak']
		res_2['avg_oldpeak'] = res_2['avg_oldpeak']/sum(res_2['avg_oldpeak'])
		res_2 = res_2.set_index('heart_disease')

		dist = 1 - get_utility(res_1['avg_oldpeak'],res_2['avg_oldpeak'])
		with open('results/avg_oldpeak_distance.csv', 'a', newline='') as f:
			fields = [i*100, dist]
			print(fields)
			writer = csv.writer(f)
