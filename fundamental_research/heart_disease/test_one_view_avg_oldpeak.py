...
for i in mlist:
	for j in range(100):
		...
		q1="select num, avg(oldpeak) from df where cp = 'typical_angina' and num is not null group by num order by num;"
		q2="select num, avg(oldpeak) from di1 where cp = 'typical_angina' and num is not null group by num order by num;"
		...
		with open('results/distance.csv', 'a', newline='') as f:
			fields = [i*100, dist]
			print(fields)
			writer = csv.writer(f)
