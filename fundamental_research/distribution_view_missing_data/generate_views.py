...
mlist = [0.05, 0.1,0.2,0.3,0.4] 

#for k in k_list:
ideal_topk  = generate_topk_views(k, df)
ideal_topk.to_csv('results/ideal_views.csv', index=False)
...
for i in mlist:
    for j in range(100):
        ...
        data['num'] = df['num']
        #print(data)
        name = str(int(i*100)) + '_' + str(j)
        missing_topk = generate_topk_views(k, data)
        missing_topk.to_csv('results/' + name + '.csv', index=False)
        print("generating missing data", name)
