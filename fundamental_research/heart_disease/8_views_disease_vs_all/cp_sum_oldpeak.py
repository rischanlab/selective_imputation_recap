...
for i in mlist:
	for j in range(100):
		...
		q1="select cp, sum(oldpeak) from df where num = 'disease' and cp is not null and oldpeak is not null group by cp order by cp;"
		q2="select cp, sum(oldpeak) from di1 where num = 'disease' and cp is not null and oldpeak is not null group by cp order by cp;"
