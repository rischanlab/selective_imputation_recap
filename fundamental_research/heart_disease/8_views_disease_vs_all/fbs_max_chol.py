...
for i in mlist:
	for j in range(100):
		...
		q1="select fbs, max(chol) from df where num = 'disease' and fbs is not null and chol is not null group by fbs order by fbs;"
		q2="select fbs, max(chol) from di1 where num = 'disease' and fbs is not null and chol is not null group by fbs order by fbs;"
